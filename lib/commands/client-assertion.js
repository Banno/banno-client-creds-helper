...
export default async function clientAssertion(clientId, privateKeyPath) {
  ...
  const enterpriseOidcTokenUri = 'https://login.jackhenry.com/a/oidc-provider/api/v0/token';
  ...
  process.stdout.write(`To obtain an access token, make a POST to the ${chalk.yellow('https://login.jackhenry.com/a/oidc-provider/api/v0/token')} endpoint.
